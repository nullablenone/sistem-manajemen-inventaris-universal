...
@section('content')
    ...
    <div class="card my-8 shadow-md border border-gray-250 bg-white rounded-lg">
        ...
        <div class="card-header flex flex-row justify-between items-center lg:flex-col lg:items-start gap-6 p-6 border-b border-gray-200">
            <div>
                <h1 class="h5 font-extrabold text-gray-800 m-0">Barang Masuk (Inbound / Purchase)</h1>
                <p class="text-xs text-gray-500 mt-1.5">Mencatat pergerakan barang masuk ke dalam gudang dari supplier.</p>
            </div>
            
            <div class="flex flex-row items-center gap-6 lg:w-full lg:justify-between">
                <!-- Search Form -->
                <form action="{{ route('stock.inbound.index') }}" method="GET" class="flex items-center">
                    <div class="relative">
                        <input type="text" name="search" value="{{ $search }}" 
                                placeholder="Cari nomor transaksi / catatan..." 
                                class="p-2 pl-8 border border-gray-200 rounded text-sm bg-gray-50 focus:outline-none focus:border-teal-500 w-72 lg:w-full transition-colors">
                        <i class="fad fa-search absolute left-3 top-3 text-gray-400 text-xs"></i>
                    </div>
                    @if($search)
                        <a href="{{ route('stock.inbound.index') }}" class="ml-2 text-xs text-gray-500 hover:text-red-500 transition-colors">Clear</a>
                    @endif
                </form>

                <!-- Add Button -->
                <a href="{{ route('stock.inbound.create') }}" class="btn-bs-primary flex items-center gap-2 py-2 px-4 shadow hover:opacity-90 transition-opacity bg-teal-600 text-white rounded font-medium text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($transactions as $tx)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 font-mono font-bold text-teal-700 text-xs">{{ $tx->transaction_number }}</td>
                                ...
                                <ul class="list-disc pl-4 space-y-1 text-xs">
                                    @foreach($tx->items as $item)
                                        ...
                                        <li>
                                            ...
                                            @endif
                                            : <span class="bg-teal-100 text-teal-800 px-1.5 py-0.5 rounded font-bold ml-1">+{{ $item->quantity }} pcs</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center gap-2">
                                    <i class="fad fa-arrow-alt-down text-4xl text-gray-300"></i>
                                    <p class="font-semibold text-base text-gray-700">Belum Ada Transaksi Masuk</p>
                                    <p class="text-xs text-gray-400">Silakan catat barang masuk baru untuk menambah persediaan stok.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
