...
@section('content')
    <div class="max-w-4xl mx-auto my-8">
        ...
        <div class="card shadow-md border border-gray-250 bg-white rounded-lg">
            <div class="card-header p-6 border-b border-gray-200">
                <h1 class="h6 font-extrabold text-gray-800 m-0">Edit Product</h1>
                <p class="text-xs text-gray-500 mt-1.5">Modify product details, category, and update variant matrix</p>
            </div>
                ...
                <form action="{{ route('products.update', $product->id) }}" method="POST" id="editProductForm">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="4"
                                  ...
                                  placeholder="Describe the product details, benefits, material, etc...">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-8 flex items-center gap-3 bg-teal-50 p-4 border border-teal-100 rounded-lg shadow-sm">
                        <input type="checkbox" name="has_variant" id="has_variant" value="1" {{ old('has_variant', $product->has_variant) ? 'checked' : '' }}
                    ...
                    @php
                        // Fetch first variant for pre-populating single mode if it is single product
                        $firstVariant = !$product->has_variant ? $product->variants->first() : null;
                        
                        // Extract all value IDs linked to current product variants
                        $activeValueIds = $product->has_variant 
                            ? $product->variants->flatMap(fn($v) => $v->attributeValues)->pluck('id')->unique()->toArray() 
                            : [];
                    @endphp
                    ...
                    <div id="singleProductSection" class="grid grid-cols-2 gap-6 mb-8">
                        <div>
                            ...
                            <input type="text" name="single_sku" id="single_sku" value="{{ old('single_sku', $firstVariant->sku ?? '') }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="single_price" id="single_price" step="0.01" value="{{ old('single_price', $firstVariant->price ?? '') }}"
                                    ...
                                    <div class="bg-gray-50 border border-gray-200 p-5 rounded-lg">
                                        ...
                                        <div class="flex flex-wrap gap-3">
                                            @foreach($attribute->values as $value)
                                                @php
                                                    $checked = in_array($value->id, $activeValueIds) ? 'checked' : '';
                                                @endphp
                    ...
                    <div class="flex items-center gap-4 border-t border-gray-200 pt-6 mt-8">
                        <button type="submit" class="btn-bs-primary py-2 px-6 shadow hover:opacity-90 transition-opacity bg-teal-600 text-white rounded font-semibold text-sm">
                            <i class="fad fa-save mr-2"></i> Update Product
                        </button>
    ...
    <script>
        // Load pre-existing variants for smart combination mapping on page edit load
            ...
            function generateVariantCombinations() {
                ...
                if (combinations.length > 0 && groupArrays.length > 0) {
                    ...
                    combinations.forEach((combination, index) => {
                        ...
                        // Extract array of integer IDs from current combination
                        const comboIds = combination.map(item => item.id).sort((a,b) => a - b).join(',');

                        // Search existing variants for a match
                        const match = existingVariants.find(ev => {
                            const evIds = ev.attribute_values.map(id => parseInt(id)).sort((a,b) => a - b).join(',');
                            return evIds === comboIds;
                        });
                        ...
                        if (match) {
                            ...
                            skuValue = `${skuPrefix}-${label.toUpperCase().replace(/\s+/g, '')}-${index + 1}`;
                        }
                        ...
                        tr.innerHTML = `
                            ...
                            <td class="px-6 py-4">
                                <input type="text" name="variants[${index}][sku]" value="${skuValue}" 
                                       placeholder="SKU" 
                                       class="w-full p-2 border border-gray-200 rounded text-sm bg-gray-50 focus:outline-none focus:border-teal-500 font-mono">
                            </td>
                            <td class="px-6 py-4">
                                <input type="number" name="variants[${index}][price]" value="${priceValue}" required step="0.01" 
